...
import com.qe.api.commoncore.ExecutionContext;
...
public class Configurator {
	...
	private void setOptionalParameter(Map<String, String> params) {
		...
		// whether to update test results into XRAY or not
		parameterMap.put(ContextConstant.PUBLISH_RESULTS_TO_XRAY, "false");// default

		// set to true if true in testNG XML
		if (params.containsKey(ContextConstant.PUBLISH_RESULTS_TO_XRAY)
				&& params.get(ContextConstant.PUBLISH_RESULTS_TO_XRAY).equalsIgnoreCase("true"))
			parameterMap.put(ContextConstant.PUBLISH_RESULTS_TO_XRAY, "true");

		// Read TestPlan and TestExecution Key from parameterMap(testNG XML)
		parameterMap.put(ContextConstant.JIRA_TESTPLAN_KEY,
				params.containsKey(ContextConstant.JIRA_TESTPLAN_KEY) ? params.get(ContextConstant.JIRA_TESTPLAN_KEY)
						: null);
		parameterMap.put(ContextConstant.JIRA_TESTEXECUTION_KEY,
				params.containsKey(ContextConstant.JIRA_TESTEXECUTION_KEY)
				? params.get(ContextConstant.JIRA_TESTEXECUTION_KEY)
						: null);
